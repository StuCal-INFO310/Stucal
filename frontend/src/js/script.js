...
function generateWeekTable(calendarEvents) {
  ...
  daysOfWeek.forEach((day) => {
    ...
    const eventList = events
      .map((event) => `<li>${event.time} - ${event.info}</li>`)
      .join("");
  ...
  document.getElementById("printable-calendar").innerHTML = calendarHTML;
}

function outputWeekTable() {
  const inputText = document.getElementById("input-text").value.trim(); // Get the input text from the textarea
  const lines = inputText.split("\n"); // Split the input text into lines

  // Regular expression patterns to match day of the week and event information
  const dayPattern = /^\w+day$/;
  const eventPattern = /^\d+\s+\w+\s+\d+\s+(\d+:\d+)-(\d+:\d+)\s+(.+)/;

  // Initialize calendarEvents object to store events for each day of the week
  ...
  lines.forEach((line) => {
    const dayMatch = line.match(dayPattern); // Match day of the week
    const eventMatch = line.match(eventPattern); // Match event

    if (dayMatch) {
      currentDay = dayMatch[0]; // Store current day of the week
    } else if (eventMatch) {
      const timeStart = eventMatch[1];
      const timeEnd = eventMatch[2];
      const eventDetails = eventMatch[3];

      // Add event to the calendarEvents object
      if (currentDay) {
        calendarEvents[currentDay].push({
  ...
  // Generate the calendar with events
  generateWeekTable(calendarEvents);
}
